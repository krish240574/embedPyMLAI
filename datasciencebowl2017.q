...
np:.p.import`numpy
/ Utility method to get a foreign pointer to n-dim arrays
/ Workaround for the tuple problem - ndim arrays get converted to tuples in python - lose all np.array methods (n>1)
npar:{np[`array;>;x]};

/ resize helper function - this actually calls the cv2 resize function
cv:.p.import`cv2;
rsz:{tup:.p.eval"tuple([50,50])";:cv[`resize;<;npar x;tup`.]};

imgs:()
fig:()
/ Process, resize and reshape input data - this function
pd:{
        show x;
        ret:0;
        .p.set[`x;x];
...
/ Pass those 32 features to another conv 3D layer, with dimension(5x5x5) filters, compute 64 features
/ pass those 64 features to a dense(fully-connected) layer

/ Import tensorflow here and then begin:
tf:.p.import `tensorflow
keras:.p.import `keras.backend; / for getting values of tensors

/ Utility method to get a value of a tf tensor
getval:{keras[`get_value;<;x]}
/ Utility method for tf.Variable - too verbose
...
/ Add prediction to tf graph explicitly
/ this is because I'm not sure if calculating prediction inside q will update the tf 
/ graph with the same value, or even create a node for that
...
/ prediction has one extra - one label is missing
if[(count prediction) <> (count k1); prediction:prediction[til count k1]];
/ apply softmax to calculate cost/loss
...
/ this will get all the weights and biases above, initialised - the node actually computes now
...
runepochs:{
        / Run training code over same inputs for multiple epochs
...
show "Running for 5 epochs now...";
runepochs each til 5;

/ runepochs each til 50; / uncomment for 50 epochs
